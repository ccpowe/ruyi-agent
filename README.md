...
- `OPENROUTER_API_KEY`：默认 `config/agents.toml` 使用 OpenRouter。
- `TELEGRAM_BOT_TOKEN`：仅 Telegram 模式需要。
- `FEISHU_APP_ID` / `FEISHU_APP_SECRET`：仅 Feishu/Lark 模式需要。
...
需要先启动 Gateway，并设置 `TELEGRAM_BOT_TOKEN`。本机默认 Gateway 可直接使用 `.env.example` 中的 `GATEWAY_BASE_URL` 和 `dev-token`。
...
需要先启动 Gateway，并设置 `FEISHU_APP_ID` 和 `FEISHU_APP_SECRET`。本机默认 Gateway 可直接使用 `.env.example` 中的 `GATEWAY_BASE_URL` 和 `dev-token`。Feishu 当前使用 WebSocket 长连接；默认是私聊模式，群聊场景需要设置 `FEISHU_GROUP_POLICY=open`，并保留 `FEISHU_REQUIRE_MENTION=true` 与 bot identity 或访问白名单。
