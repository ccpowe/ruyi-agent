...
_TEAM_CONSOLE_HTML_HEADERS = {
    **_TEAM_CONSOLE_COMMON_HEADERS,
    "Content-Security-Policy": _TEAM_CONSOLE_CSP,
}
_TEAM_CONSOLE_LOGIN_HEADERS = {
    **_TEAM_CONSOLE_COMMON_HEADERS,
    "Content-Security-Policy": _TEAM_CONSOLE_LOGIN_CSP,
}


def _team_console_login_page(*, error: str | None = None) -> str:
...
def _team_console_login_response(
    *,
    status_code: int = 200,
    error: str | None = None,
) -> HTMLResponse:
    headers = dict(_TEAM_CONSOLE_LOGIN_HEADERS)
    if status_code == 401:
        headers["WWW-Authenticate"] = 'Bearer realm="ruyi-team-console"'
    return HTMLResponse(
        _team_console_login_page(error=error),
        status_code=status_code,
        headers=headers,
    )


def _raw_request_header_values(request: Request, name: bytes) -> list[bytes]:
    return [
        value
...
def _form_content_type_is_supported(request: Request) -> bool:
    values = _raw_request_header_values(request, b"content-type")
    if len(values) != 1:
        return False
    try:
...
async def _read_team_console_login_token(
    request: Request,
) -> tuple[str | None, int | None]:
    if not _form_content_type_is_supported(request):
        return None, 415

    lengths = _raw_request_header_values(request, b"content-length")
    if len(lengths) > 1:
        return None, 400
    if lengths:
        try:
...
def attach_gateway_routes(
    ...
    *,
    ...
    async def require_bearer(request: Request) -> None:
        auth_source = console_auth.authenticate_api_request(request.scope)
        ...
        if auth_source == "console_session":
            console_auth.mark_console_api_authenticated(request.scope)

    def console_session_is_valid(request: Request) -> bool:
    ...
    async def team_console_login(request: Request) -> Response:
        if request.url.query:
            return _team_console_login_response(
                status_code=400,
                error="登录请求不能包含查询参数。",
            )
        if not console_auth.console_transport_allowed(request.scope):
            return _team_console_login_response(
                status_code=400,
                error="调试台登录要求 HTTPS；localhost 可使用 HTTP。",
            )
        if console_session_is_valid(request):
            return RedirectResponse(
                "/debug/team",
                status_code=303,
                headers=dict(_TEAM_CONSOLE_COMMON_HEADERS),
            )
        return _team_console_login_response()
    ...
    async def create_team_console_session(request: Request) -> Response:
        ...
        token, error_status = await _read_team_console_login_token(request)
        ...
        if token is None or not console_auth.verify_login_token(token):
            return _team_console_login_response(
                status_code=401,
                error="Gateway Token 无效。",
            )
        ...
        response.set_cookie(
            TEAM_CONSOLE_SESSION_COOKIE,
            console_auth.issue_session(),
            max_age=TEAM_CONSOLE_SESSION_TTL_SECONDS,
            path="/",
            secure=console_auth.session_cookie_is_secure(request.scope),
            httponly=True,
            samesite="strict",
        )
        return response
    ...
    async def team_console(request: Request) -> Response:
        ...
        if not console_session_is_valid(request):
            return RedirectResponse(
                "/debug/team/login",
                status_code=303,
                headers=dict(_TEAM_CONSOLE_COMMON_HEADERS),
        ...
        return HTMLResponse(
            ...
            headers=dict(_TEAM_CONSOLE_HTML_HEADERS),
        )
    ...
    async def handle_unexpected_error(
        ...
        headers = (
            {"Cache-Control": "no-store"}
            if console_auth.console_api_was_authenticated(request.scope)
            else None
        )
        return JSONResponse(
            status_code=500,
