...
from ruyi_agent.gateway_protocol.contracts import MAX_TASK_EVENT_CURSOR_LENGTH, TASK_EVENT_CURSOR_VERSION, InvalidTaskEventCursorError
def encode_task_event_cursor(event: Any) -> str:
    payload = json.dumps({"v": TASK_EVENT_CURSOR_VERSION, "task": event.task_id, "run": event.run_count, "event": event.event_id}, ensure_ascii=True, sort_keys=True, separators=(",", ":")).encode("utf-8")
...
def decode_task_event_cursor(cursor: str, *, task_id: str, run_count: int) -> int:
    ...
    if not isinstance(version, int) or isinstance(version, bool) or version != TASK_EVENT_CURSOR_VERSION:
        raise InvalidTaskEventCursorError("Unsupported Task event cursor version")
    bound_run = payload.get("run")
    if payload.get("task") != task_id or not isinstance(bound_run, int) or isinstance(bound_run, bool) or bound_run != run_count:
