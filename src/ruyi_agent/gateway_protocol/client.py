...
class GatewayProtocolClient:
    ...
    async def create_task(
        ...
        *,
        ...
        payload: dict[str, Any] = {
            "input": task_input_payload(content, attachments),
    ...
    async def send_input(
        ...
        *,
        ...
        return await self.request_json(
            ...
            json={"input": task_input_payload(content, attachments)},
            idempotency_key=idempotency_key,
        )
    ...
    async def list_task_messages(
        ...
        return await self.request_json(
            "GET", f"tasks/{task_id}/messages", params=params
        )
    ...
    async def open_task_event_stream(
        ...
        async with self._http.stream_task_events(
            f"tasks/{task_id}/events", run_count=run_count, last_event_id=last_event_id
        ) as events:
            yield events
    ...
    async def submit_review_decision(
        self, *, task_id: str, review_id: str, decisions: list[Mapping[str, Any]]
    ) -> dict[str, Any]:
        return await self.request_json(
            "POST",
            f"tasks/{task_id}/reviews/{review_id}/decision",
            json=review_decision_payload(decisions),
        )

    async def download_artifact_response(self, *, path: str) -> httpx.Response:
        return await self.request_raw(
            "POST", artifact_download_path(), json=artifact_download_payload(path)
        )

    async def download_task_artifact_response(
        self, *, task_id: str, artifact_id: str
    ) -> httpx.Response:
        return await self.request_raw(
            "GET", task_artifact_download_path(task_id, artifact_id)
        )


def task_input_payload(
    content: str, attachments: list[Mapping[str, Any]] | None = None
) -> dict[str, Any]:
